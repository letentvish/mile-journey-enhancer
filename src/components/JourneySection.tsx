...
const Milestone = ({ title, description, icon: Icon, color, position, index }: MilestoneProps) => {
  ...
  return (
    <div
      ref={milestoneRef}
      className={cn(
        "flex flex-col items-center opacity-0 transition-all duration-300",
    ...
    >
      <div className="relative flex flex-col items-center">
        {/* Badge */}
        <div className={cn(
          "w-16 h-16 rounded-full neumorphic flex items-center justify-center mb-2 relative z-10",
          "border border-mile-stroke"
        )}>
          <div className={cn(
      ...
      <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 max-w-xs md:max-w-sm shadow-lg border border-white/20 mt-4">
        <h4 className="font-bold text-sm text-mile-orange mb-1">{title}</h4>
...
const JourneySection = () => {
  ...
  return (
    <section id="journey" className="section bg-[#2E2E3D] text-white relative py-20">
      <h2 className="h3-heading text-center mb-4 text-mile-orange">
        Your MILE Journey: Gamified Learning with Badges
      </h2>
      
      <p className="body-regular text-center text-gray-300 max-w-3xl mx-auto mb-16">
      ...
      <div className="hidden md:block relative h-64 max-w-5xl mx-auto mb-12">
        ...
        <div className="flex justify-between absolute inset-0">
          {milestones.map((milestone, index) => (
            <div key={index} className={`relative ${index % 2 === 0 ? 'self-start' : 'self-end'}`} style={{left: `${index * 28 + 8}%`}}>
              <Milestone
                {...milestone}
                index={index}
              />
      ...
      <div className="md:hidden">
        <div className="relative w-2 bg-[#1A1A2E] mx-auto">
          ...
          {milestones.map((milestone, index) => (
            <div key={index} className="py-8 relative">
              <div className={cn("flex items-center", index % 2 === 0 ? "justify-start" : "justify-end")}>
                ...
                <div className="w-full pl-8">
                  <div className="neumorphic p-4 rounded-xl shadow-lg text-center">
                    ...
                    <h4 className="font-bold text-sm text-mile-orange mb-1">{milestone.title}</h4>
